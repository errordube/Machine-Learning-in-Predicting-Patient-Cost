---
title: "Estimating Medicare Costs for AMI"
...
#-#-# Predicting Medicare for AMI #-#-#
      ...
      ## Step 7: Build and Evaluate Artifical Neural Network  Model (Feeforward ANN)
...
final_data_AMI <- filter(final_data,DRG_Cd %in% c("246","247","248","249","250","251","280","281"))
#sum(final_data_AMI[, 'Total_discharges']) #total discharge sum = 258186


selected_data <- final_data_AMI %>%
...
ggplot(selected_data , aes(x = Ave_medical_payment)) +
  geom_histogram(binwidth = 5000, col = 'green') +
  labs(title = "Histogram of Average Medicare Payments(AMI Codes)", x = "Average Total Payments")
...
ggplot(usmap, aes(x = long, y = lat, group = group, fill = Avg_Mdcr_Pymt)) +
  ...
  labs(title = "Average Medicare Payments by State (AMI Codes)")
